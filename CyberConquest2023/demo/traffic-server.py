    ...
    def light_control_loop(self):
        while self.active:
            ...
            for i in range(len(self.pixels)):
                if len(self.light_instructions) <= i:
                    continue
                if len(self.light_instructions[i]) == 0:
                    continue
                try:
...
@app.route("/")
def hello_world():
    return render_template("index.html", currentInstruction=json.dumps(app.light_controller.light_instructions))
